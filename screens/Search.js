...
export default class SearchScreen extends Component {
    ...
    db.collection("transactions")
      ...
      .then(snapshot => {
        snapshot.docs.map(doc => {
          this.setState({
            allTransactions: [...this.state.allTransactions, doc.data()], lastVisitedTransaction: doc
          });
        });
      });
  };
  ...
  fetchMoreTransactions = async text=>{
    var enteredText = text.toUpperCase().split("");
      text.text.toUpperCase();
      const {lastVisitedTransaction, allTransactions} = this.state;
      if(enteredText[0] === "B"){
        const query = await db
        .collection("transactions")
        .where("bookId", "==", text)
        .startAfter(lastVisitedTransaction)
        .limit(10)
        .get();
        query.docs.map(doc=>{
          this.setState({
            allTransactions: [...this.state.allTransactions, doc.data()],
          lastVisitedTransaction: doc  
          });
        });
      }
        ...
        .collection("transactions")
        .where("bookId", "==", text)
        .startAfter(this.state.lastVisitedTransaction)
        .limit(10)
        .get();
        query.docs.map(doc=>{
          this.setState({
